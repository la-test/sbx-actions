Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell",
    inline: "apt-get -q clean && \
      ...
      apt-get -q clean"
  config.vm.provision "shell",
    inline: "sudo echo -n \"#{ENV['SSH_DEPLOYMENT_KEY']}\" \
      > /root/.ssh/id_ed25519 && sudo chmod 0600 /root/.ssh/id_ed25519"
  config.vm.provision "shell",
    inline: "sudo echo -n \"restrict,command=\"sudo update-deployment base-local \
      #{ENV['SSH_DEPLOYMENT_KEY_PUB']}\" >> /root/.ssh/authorized_keys"
end
